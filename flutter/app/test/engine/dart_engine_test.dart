...
void main() {
  ...
  group('convertWebp', () {
    ...
    test(
      'keeps ComicInfo.xml (in place) when removeComicInfo is false',
      () async {
        ...
        final result = await engine.convertWebp(
          ArchiveData('book.cbz', zip),
          const ConvertOptions(onlyIfSmaller: false, removeComicInfo: false),
        );
        expect(result.success, isTrue);
        final entries = collectZipEntries(result.output!);
    ...
    test(
      'drops ComicInfo.xml (the default), leaving images renumbered',
      () async {
        final zip = makeZip({
          'first.png': makeSolidPng(16, 16),
          'ComicInfo.xml': '<x/>'.codeUnits,
